...
resource "azurerm_network_interface" "bastion" {
  ...
  ip_configuration {
    name                          = "configuration"
    subnet_id                     = module.shared_svcs_network.vnet_subnets[0]
...
resource "azurerm_virtual_network_peering" "legacy-shared" {
  ...
  remote_virtual_network_id = module.shared_svcs_network.vnet_id
}